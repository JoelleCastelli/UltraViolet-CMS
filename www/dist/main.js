...
function sliderInit(slider) {
  ...
  container.addClass('slides-container'); // Mettre le contenu du slider dans notre container de slides

  var sliderContent = slider.html();
  container.html(sliderContent); // Ajouter la classe slide à toutes les images

  container.children('img').addClass('slide'); // Mettre le container de slides dans slider

  slider.html(container); // Ajouter une nav au slider

  var nav = $('<nav></nav>');
  nav.append('<button class="prev">prev</button>');
  nav.append('<button class="next">next</button>');
  slider.append(nav);
  slider.attr('data-currentSlide', '0'); //Ajouter un écouteur de clic sur les boutons prev et next
